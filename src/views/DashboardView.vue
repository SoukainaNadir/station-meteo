<template>
  <v-container fluid class="dashboard-container">
    ...
    <!-- Station Selector with Map Preview -->
    <v-row class="mt-4">
      <v-col cols="12" md="8">
      ...
      <v-col cols="12" md="4">
        <v-card elevation="3" class="station-selector-card">
          ...
          <v-card-text class="pa-4">
            ...
            <div v-if="selectedStation" class="station-info mt-4 pa-3 bg-grey-lighten-5 rounded">
              ...
              <div class="d-flex align-center">
                <v-icon size="small" class="mr-2" color="grey-darken-1">mdi-clock-outline</v-icon>
                <span class="text-caption text-grey-darken-2">
        ...
        <v-card-text class="pt-4">
          <v-list>
            <v-list-item>
              ...
              <v-list-item-title>
                {{ selectedMapStation.measurements?.temperature?.value?.toFixed(2) }}°C
              </v-list-item-title>
            ...
            <v-list-item>
              ...
              <v-list-item-title>
                {{ selectedMapStation.measurements?.pressure?.value?.toFixed(1) }} hPa
              </v-list-item-title>
            ...
            <v-list-item>
              ...
              <v-list-item-title>
                {{ selectedMapStation.measurements?.wind_speed_avg?.value?.toFixed(1) }} Kts
              </v-list-item-title>
            ...
            <v-list-item>
              ...
              <v-list-item-title>
                {{ selectedMapStation.measurements?.rain?.value?.toFixed(1) }} mm
              </v-list-item-title>
...
const initMap = () => {
  ...
  meteoStore.sondes.forEach(sonde => {
    ...
    const feature = new Feature({
      geometry: new Point(fromLonLat([sonde.location.long, sonde.location.lat])),
      name: sonde.name,
      id: sonde.sonde_id,
      temperature: temp
    })
  ...
  const centerLon = meteoStore.sondes.reduce((sum, s) => sum + s.location.long, 0) / meteoStore.sondes.length
  const centerLat = meteoStore.sondes.reduce((sum, s) => sum + s.location.lat, 0) / meteoStore.sondes.length
...
const loadChartData = async (period = { value: '24h', hours: 24 }) => {
  ...
  try {
    const [temp, hum, press, rain] = await Promise.all([
      meteoStore.fetchArchiveDataByPeriod(selectedStationId.value, period, 'temperature'),
      meteoStore.fetchArchiveDataByPeriod(selectedStationId.value, period, 'humidity'),
      meteoStore.fetchArchiveDataByPeriod(selectedStationId.value, period, 'pressure'),
      meteoStore.fetchArchiveDataByPeriod(selectedStationId.value, period, 'rain')
    ])
